...
class Environment():
	...
	def update(self, object_id, position, velocity):
		...
		try:
			...
			if __debug__ and debug: self.logger.debug("Moving %s %f from %s to %s" % (object_name,
			                                                                          update_distance,
			                                                                          self.map[object_id].position,
			                                                                          position))
			self.map[object_id] = map_entry(object_id, position, velocity, object_name)
		except KeyError:
			if __debug__ and debug: self.logger.debug("Creating map entry for %s at %s" % (object_name, position))
			self.map[object_id] = map_entry(object_id, position, velocity, object_name)
