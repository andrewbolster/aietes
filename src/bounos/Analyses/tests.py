...
class TestTrustGenerationEquality(TestCase):
    ...
    def setUpClass(cls):
        ...
        with pd.get_store(control_case_path+'.h5') as store:
            tf = store.trust
        cls.sample = tf.xs('CombinedTrust', level='var', drop_level=False).xs(0, level='run', drop_level=False).xs('Bravo', level='observer', drop_level=False)
    
    def testUnweighted(self):
        sample_weight = None

        tp_as_matrix = Trust.generate_node_trust_perspective(self.sample, metric_weights=sample_weight, par=False, as_matrix=True)
        tp = Trust.generate_node_trust_perspective(self.sample, metric_weights=sample_weight, par=False, as_matrix=False)
        assert_frame_equal(tp,tp_as_matrix)
        tpp_as_matrix = Trust.generate_node_trust_perspective(self.sample, metric_weights=sample_weight, par=True, as_matrix=True)
        assert_frame_equal(tpp_as_matrix,tp_as_matrix)
        tpp = Trust.generate_node_trust_perspective(self.sample, metric_weights=sample_weight, par=True, as_matrix=False)
