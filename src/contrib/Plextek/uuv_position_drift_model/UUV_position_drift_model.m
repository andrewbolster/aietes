...
% monte-Carlo loop iteration
    ...
    for jj = 1:numloops    
      ...
      bias_along = 0;
      bias_across = 0;
      Gyro_scale_rand_term = rand(1,1);% Random Gaussian error onto bias - constant during a monte-carlo loop, but different between them
      DVL_scale_rand_term  = rand(1,1);  %Starting value of scale error
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  tc_DVL_cn = 1800;     % time constant for markov process
    ...
    if count == 2  % on first iteration of simulation, set the bias
        ran = rand(1,1);  % generate random number (0,1)  = mean 0, std 1;
